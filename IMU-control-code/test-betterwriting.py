...
def IMU_on(environ): # starts the IMU recording
    ...
    stringdate = "UTCtime:**"+str(datenow[0])+","+str(datenow[1])+","+str(datenow[2])+","+str(datenow[3])+":"+str(datenow[4])+":"+str(datenow[5])+cardname
...
web_app.on("GET", "/triangle", triangle)
web_app.on("GET", "/M", M)
web_app.on("GET", "/DT", DT)
web_app.on("GET", "/beaker", beaker)
web_app.on("GET", "/parasail", parasail)
web_app.on("GET", "/jetski", jetski)
web_app.on("GET", "/pentagon", pentagon)
