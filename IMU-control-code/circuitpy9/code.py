...
from adafruit_esp32spi import adafruit_esp32spi, adafruit_esp32spi_socketpool
...
print("Arduino Nano RP2040 Connect webclient setting up")
...
for ap in esp.scan_networks():
    print(str(ap.ssid), "strength", ap.rssi)
...
while not time_set:
    if secrets["ssid"]!="tufts_eecs":
        ...
        #tufts_ntp.set_ntp_time(esp)
            ...
            ntp = adafruit_ntp.NTP(pool, server="time.eecs.tufts.edu",port=123)
            now = ntp.datetime
            time_set=True
        except ValueError:
            time_set = False
